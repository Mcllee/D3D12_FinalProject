...
void CGameFramework::CreateDirect3DDevice()
{
	...
	/*
	펜스와 동기화를 위한 이벤트를 객체를 생성한다(이벤트 객체의 초기값을 FALSE이다). 이벤트가 실행되면(Signal)
...
void CGameFramework::CreateCommandQueueAndList()
{
	...
	// 명령 리스트는 생성되면 열린(open) 상태이므로 닫힌(close) 상태로 만든다.
}
...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case VK_ESCAPE:
		...
		break;
	case 'R':
		m_pPlayer->SetPosition(XMFLOAT3(1000.0f, 10.0f, 50.0f));
		break;
...
void CGameFramework::ProcessInput()
{
	...
	if (::GetKeyboardState(pKeyBuffer))
	{
		...
		if (pKeyBuffer[VK_SPACE] & 0xF0) dwDirection |= DIR_FORWARD;
	}
	...
	것은 마우스 버튼이 눌려진 상태를 의미한다. 마우스 버튼이 눌려진 상태에서 마우스를 
	좌우 또는 상하로 움직이면 플레이어를 x-축 또는 y-축으로 회전한다.*/
