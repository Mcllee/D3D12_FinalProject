...
void CSpaceShipCamera::Rotate(float x, float y, float z)
{
	if (m_pPlayer && (x != 0.0f))
	{
		...
		XMFLOAT3 a = m_pPlayer->GetPosition();

		m_xmf3Position = Vector3::Subtract(m_xmf3Position, a);
...
void CThirdPersonCamera::Update(XMFLOAT3& xmf3LookAt, float fTimeElapsed)
{
	...
	회전하는 효과를 구현하기 위한 것이다. m_fTimeLag가 1보다 크면 fTimeLagScale이 작아지고 실제 회전(이동)이 적게 일어날 것이다. 
...
void CThirdPersonCamera::SetLookAt(XMFLOAT3 xmf3LookAt)
{
	...
	XMFLOAT3 a = m_pPlayer->GetUpVector();

	XMFLOAT4X4 mtxLookAt = Matrix4x4::LookAtLH(m_xmf3Position, xmf3LookAt, a);
