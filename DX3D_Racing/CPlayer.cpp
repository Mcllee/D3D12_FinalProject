...
void CPlayer::Move(DWORD dwDirection, float fDistance, bool bUpdateVelocity)
{
	...
	else if (dwDirection)
	{
		...
		prev_Position = m_xmf3Position;	// 움직이기 전 값을 미리 저장
...
void CPlayer::Move(const XMFLOAT3& xmf3Shift, bool bUpdateVelocity)
{
	...
	else
	{
		...
		// if (m_pCamera) m_pCamera->Move(xmf3Shift);	-> 항상 카메라가 있으므로 필요없다.
...
void CPlayer::Update(float fTimeElapsed)
{
	...
	/*플레이어의 속도 벡터의 y-성분의 크기를 구한다. 이것이 y-축 방향의 
	최대 속력보다 크면 속도 벡터의 y-방향 성분을 조정한다.*/
	...
	구하고 단위 벡터로 만든다. 마찰 계수를 시간에 비례하도록 하여 마찰력을 구한다. 
...
CCamera* CPlayer::OnChangeCamera(DWORD nNewCameraMode, DWORD nCurrentCameraMode)
{
	...
	Up 벡터를 월드좌표계의 y-축 방향 벡터(0, 1, 0)이 되도록 한다. 즉, 똑바로 서도록 한다. 그리고
	스페이스-쉽 카메라의 경우 플레이어의 이동에는 제약이 없다. 특히, y-축 방향의 움직임이 자유롭다.
	그러므로 플레이어의 위치는 공중(위치 벡터의 y-좌표가 0보다 크다)이 될 수 있다. 이때 새로운 카메라가 1인칭 또는 3인칭 카메라이면 플레이어의
...
Up 벡터가 두 번째 행 벡터, Look 벡터가 세 번째 행 벡터, 플레이어의 위치 벡터가 네 번째 행
벡터가 된다.*/
void CPlayer::OnPrepareRender()
{
	...
	UpdateTransform(NULL); //-> 캐릭터가 날라가버림
}
...
void CTerrainPlayer::OnPlayerUpdateCallback(float fTimeElapsed)
{
	...
	float fHeight = pTerrain->GetHeight(xmf3PlayerPosition.x, xmf3PlayerPosition.z) + 6.0f;	// 현재 맵 위치의 높이
